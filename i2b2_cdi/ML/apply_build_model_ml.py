...
from i2b2_cdi.ML.mlHelper import load_pred_facts,compute_classification_metrics,serialize_model_to_base64, load_model_from_base64, clean_json_string, convert_numpy_types, get_tuple_from_df, format_data_paths,get_installed_packages, get_top_n_features, save_tuple,load_tuple
from i2b2_cdi.utils.patient_set import get_patient_set_instance_id

from i2b2_cdi.ML.build_model_ML_helper import get_concept_blob,make_pipeline,save_model_in_concept_blob
...
#%%
def apply_build_model(conceptPath,ml_code,crc_ds,job_id,blob=None, concept_blob=None,input_params=None):
    '''
...
#%%
    ...
    if params['task']=='build':
        ...
        y_pred=pd.Series(loaded_model.predict(X_selected))
        print('predicted',y_pred.value_counts())
        elapsed_time=time.time() - params['start_time']
        print('elapsed time:{} secs'.format(str(elapsed_time)))
        
        load_pred_facts(y_pred,pt_num_list,ml_code)
